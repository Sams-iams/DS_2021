...
Load in data and rename PAM column: ***we need new dataset name here****
```{r}
...
making boxplot to visualize data for chamberlanium:
```{r}
...
cham_plot
```
boxplot to visualize calliarthron data:
```{r}
...
ggsave(pam_plots_fig1, filename = "pam_fig1.png", height = 10, width = 15)
```
